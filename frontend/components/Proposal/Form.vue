      ...
      el-form(autocomplete="off" @submit.prevent="onSubmit" ref="myForm" label-position="top")
        .card.m-auto.bg-white.p-5.rounded-3xl(class="w-[100%]")
          .grid.grid-cols-1(class="md:grid-cols-2 gap-[16px]", style="height: 80% ")
            InputSelect(v-if="!editStatus" label="Related type" name="relatedEntityType" :options="proposalRelatedTypes" innerClass="!mb-0" :value="data?.relatedEntityType" @change="[fetchRelatedToOptions($event.value), setFieldValue('relatedEntityId', '')]")
            InputSelect(v-if="!editStatus" label="Related to" name="relatedEntityId" :options="proposalRelatedToOptions" innerClass="!mb-0" :value="data?.relatedEntityId")
            InputText(v-if="!editStatus" label="Proposal Title" name="title" :value="data?.title")
            InputText(v-if="!editStatus" label="Proposal Version" name="version" type="number" step="0.1" min="1" :value="data?.version ?? '1.0'")
            InputSelect(v-if="!editStatus" :value="data?.type" label="Proposal Type" name="type" :options="ProposalType" innerClass="!mb-0")
            InputSelect(v-if="editStatus" :value="data?.status == 'REJECTED' ? 'reject' : 'approve'" label="Proposal Status" name="status" :options="optionStaus" innerClass="!mb-0")
            ...
            InputSelect(v-if="!editStatus" :value="data?.users?.map((el) => el?.id)" label="Assign User" name="users" :options="mappedUsers" isMultiple innerClass="!mb-0")
            InputUploadImage.col-span-1(v-if="!editStatus" label="Company Logo" name="companyLogo" class="col-span-2 md:col-span-2" :value="data?.companyLogo" style="justify-items:center" note="max File Size 20mg")
            InputText.col-span-1.item-center(v-if="!editStatus" label="Notes" name="notes" type="textarea" class="md:col-span-2" :value="data?.notes")
            InputText.col-span-1.item-center(v-if="values?.status == 'reject' && editStatus" label="Reason" name="reason" type="textarea" class="md:col-span-2" :value="data?.rejectionReason")
            InputUploadFiles.col-span-1(v-if="!editStatus" label="File Attachments" name="file" :limit="4" :formats="fileAttachmentsFormats" :value="data?.fileAttachments?.map((file: any) => ({name: file, response: file }))" formatsError="Accept only PDF, Word, Jpg, Jpeg, Png" :tipNote="false" class="md:col-span-2" innerClass="!mb-0")
          el-form-item.mt-8(class="!mb-0")
            el-button(size="large" plain type="primary" class="!rounded-2xl" @click="dialog = false") Cancel
